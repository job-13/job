...
input double  InpMaxSpread        = 2.0;       // Max spread (price units)
input double  InpMaxDailyDD       = 4.0;        // Max daily drawdown % — hard stop for the day

input group "=== SL / TP ==="
input int     InpATRPeriod        = 14;         // ATR period (M15)
input double  InpSLMultiplier     = 1.0;       // SL = ATR x this — 0.25 gives Gold room to breathe
input double  InpMinSL            = 8.70;       // Min SL in $ — never tighter than this on Gold

input group "=== BREAKEVEN ==="
input bool    InpUseBreakeven     = true;       // Enable breakeven
input double  InpBEMultiplier     = 2.85;       // Move SL to entry when profit >= ATR x this

input group "=== TRAILING STOP ==="
input bool    InpUseTrail         = true;       // Enable trailing stop
input double  InpTrailStartMult   = 5.12;       // Start trailing when profit >= ATR x this
input double  InpTrailStepMult    = 0.20;       // Trail moves ATR x this per step

input group "=== LOSS PROTECTION ==="
input int     InpMaxConsecLosses  = 11;          // Losses before cooldown
input int     InpCooldownMins     = 30;         // Cooldown minutes after loss streak
input int     InpPostCloseCooldown = 136;        // Minutes to wait after ANY close before re-entering
...
input group "=== SIGNAL ==="
input int     InpEMAFast          = 41;         // M15 fast EMA
input int     InpEMASlow          = 306;         // M15 slow EMA
